...
1. `behavioral-pattern-agent`: highest priority for changing business rules, workflow branching, state transitions, commands, notifications, validation chains, or algorithms.
2. `structural-pattern-agent`: highest priority for incompatible APIs, wrapper/decorator needs, facades, persistence boundaries, or object tree composition.
3. `creational-pattern-agent`: highest priority for constructor complexity, object families, optional fields, immutable setup, or controlled lifecycle.
4. `architectural-pattern-agent`: highest priority for application-level boundaries, web entry flow, distributed transactions, DTO boundaries, gateway/filter/page-object concerns.
