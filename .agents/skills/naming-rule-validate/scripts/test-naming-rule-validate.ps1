...
try {
    ...
    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/skill-maintenance-review/scripts') -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $tempRoot 'scripts') -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/workflow-validate/scripts') -Force | Out-Null
...
---
name: java-review
description: Use when reviewing oava backend code.
---

# oava Review
'@
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/java-review/subagents/java-api-contract-review.md') -Encoding utf8 -Value '# Valid subagent'
...
'@
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/skill-maintenance-review/scripts/add-skill-feedback.ps1') -Encoding utf8 -Value '# valid add script'
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/skill-maintenance-review/scripts/apply-skill-upgrade-proposal.py') -Encoding utf8 -Value '# valid apply script'
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/workflow-validate/scripts/validate-workflow.ps1') -Encoding utf8 -Value '# valid skill-local script'
    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/diagram-wireframe-generate') -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/diagram-wireframe-generate/SKILL.md') -Encoding utf8 -Value @'
---
...
'@
    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/test-automation-validate') -Force | Out-Null
...
---
name: java-analyze
description: Use when analyzing oava backend architecture.
---
...
'@
    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/code-design-pattern') -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/code-design-pattern/SKILL.md') -Encoding utf8 -Value @'
---
...
# Code Design Pattern
'@
    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/skill-maintenance-review') -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/skill-maintenance-review/SKILL.md') -Encoding utf8 -Value @'
---
...
# Invalid
'@
    & powershell -NoProfile -ExecutionPolicy Bypass -File $validator -Root $tempRoot -Paths '.agents/skills/clean-code/SKILL.md' | Out-Null
    Assert-True ($LASTEXITCODE -eq 1) 'Naming validator should reject noun-only capability names.'

    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/wireframe-diagram-generate') -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/wireframe-diagram-generate/SKILL.md') -Encoding utf8 -Value @'
---
...
# Invalid
'@
    & powershell -NoProfile -ExecutionPolicy Bypass -File $validator -Root $tempRoot -Paths '.agents/skills/wireframe-diagram-generate/SKILL.md' | Out-Null
    Assert-True ($LASTEXITCODE -eq 1) 'Naming validator should reject qualifier-domain-action ordering.'

    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/activity-diagram-generate') -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/activity-diagram-generate/SKILL.md') -Encoding utf8 -Value @'
---
...
# Invalid
'@
    & powershell -NoProfile -ExecutionPolicy Bypass -File $validator -Root $tempRoot -Paths '.agents/skills/activity-diagram-generate/SKILL.md' | Out-Null
    Assert-True ($LASTEXITCODE -eq 1) 'Naming validator should reject diagram qualifier before the diagram domain.'

    New-Item -ItemType Directory -Path (Join-Path $tempRoot '.agents/skills/automation-test-validate') -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $tempRoot '.agents/skills/automation-test-validate/SKILL.md') -Encoding utf8 -Value @'
---
...
# Invalid
'@
    & powershell -NoProfile -ExecutionPolicy Bypass -File $validator -Root $tempRoot -Paths '.agents/skills/automation-test-validate/SKILL.md' | Out-Null
    ...
    foreach ($invalidName in @('architect-onion-design', 'java-design', 'onion-pattern-design', 'pattern-design', 'pattern-onion-design', 'qa-test-review', 'shared-code-design')) {
        New-Item -ItemType Directory -Path (Join-Path $tempRoot ".agents/skills/$invalidName") -Force | Out-Null
        Set-Content -LiteralPath (Join-Path $tempRoot ".agents/skills/$invalidName/SKILL.md") -Encoding utf8 -Value @"
---
name: $invalidName
...
# Invalid
"@
        & powershell -NoProfile -ExecutionPolicy Bypass -File $validator -Root $tempRoot -Paths ".agents/skills/$invalidName/SKILL.md" | Out-Null
    ...
    Set-Content -LiteralPath (Join-Path $tempRoot 'scripts/oavaReview.ps1') -Encoding utf8 -Value '# invalid script'
    & powershell -NoProfile -ExecutionPolicy Bypass -File $validator -Root $tempRoot -Paths 'scripts/oavaReview.ps1' | Out-Null
