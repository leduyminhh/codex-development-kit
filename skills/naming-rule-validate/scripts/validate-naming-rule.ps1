...
function Test-ScriptName {
    ...
    $scriptVerbs = @(
        ...
        'run', 'test', 'install', 'resolve', 'invoke', 'add', 'apply'
    )
...
function Get-NameFromPath {
    ...
    if ($relativePath -match '^skills/[^/]+/subagents/([^/]+)\.md$') {
    ...
    if ($relativePath -match '^skills/[^/]+/scripts/([^/]+)\.(ps1|py)$') {
...
function Get-TargetFiles {
    ...
    foreach ($searchRoot in @('.codex/agents', '.codex/hooks', 'skills', 'workflows', 'scripts')) {
