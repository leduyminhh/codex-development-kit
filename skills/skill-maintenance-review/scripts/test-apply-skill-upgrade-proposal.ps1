...
try {
    ...
    New-Item -ItemType Directory -Path (Join-Path $tempRoot 'skills/demo-skill') -Force | Out-Null
...
'@
    ...
    Set-Content -LiteralPath (Join-Path $tempRoot 'skills/demo-skill/SKILL.md') -Encoding utf8 -Value "# Old content"
...
'@
    ...
    Assert-Equal '# New content' (Get-Content -LiteralPath (Join-Path $tempRoot 'skills/demo-skill/SKILL.md') -Raw).Trim() 'Apply should write updated content.'
